...
def run_strategy(prices: pd.DataFrame, spec: dict) -> dict:
    ...
    positions = np.where(prices > rolling_high, 1, 0)
    positions = np.where(prices < rolling_low, 0, positions)

    smoothed_positions = positions.rolling(window).mean()

    weights = smoothed_positions / smoothed_positions.sum(axis=1, keepdims=True)
