...
def run_strategy(prices: pd.DataFrame, spec: dict) -> dict:
    ...
    signals = (prices > rolling_high.shift(1)).astype(int)
    ...
    normalized_weights = smoothed_positions / smoothed_positions.abs().sum(axis=1) * max_leverage

    turnover = (normalized_weights.diff().abs().sum(axis=1)).mean() * 252

    returns = (prices.pct_change() * normalized_weights.shift(1)).sum(axis=1)
