...
def run_strategy(prices: pd.DataFrame, spec: dict) -> dict:
    ...
    long_entries = z_scores <= -z_entry
    short_entries = z_scores >= z_entry
    long_exits = (z_scores > -z_exit) | (z_scores * long_entries.shift(1) < 0)
    short_exits = (z_scores < z_exit) | (z_scores * short_entries.shift(1) > 0)

    positions = np.where(long_entries, 1, np.where(short_entries, -1, 0))
    positions = positions.rolling(lookback).mean()

    weights = positions / positions.abs().sum(axis=1).values[:, None]
