...
def run_strategy(prices: pd.DataFrame, spec: dict) -> dict:
    ...
    if spec["frequency"] == "weekly":
        ...
        returns_weekly = prices.pct_change()

    weights_smoothed = weights.rolling(window=params["smoothing_window"]).mean()
    weights_normalized = weights_smoothed / weights_smoothed.abs().sum(axis=1)

    portfolio_returns = (weights_normalized.shift(1) * returns_weekly).sum(axis=1)
    portfolio_returns = portfolio_returns * spec["max_leverage"]
    ...
    turnover = (weights_normalized.diff().abs().sum(axis=1)).mean() * 252
